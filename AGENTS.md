...
- `CarServer.Response.response_msg` (`proto/command/car_server.proto`) models the full server reply surface except field 12 (`GetRateTariffResponse`, blocked on a namespace-publish decision - its only field is a `com.tesla.proto.energy.rate_tariff.v1` type, an app-only namespace) and fields 17/19 (`centerdisplay.server`/`webrtc_comms`, app-only namespaces, intentionally excluded). Before adding a new reply oneof field, check whether its payload type pulls in an app-only namespace first.
