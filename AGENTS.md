...
- Sharp edge: protoletariat is installed with `pip --no-deps` (see `scripts/generate.sh`) because it over-pins `protobuf<6` while only parsing descriptor sets; its `--exclude-imports-glob` patterns match dependency names WITHOUT the `.proto` suffix. It also only recognizes protoc's `.py` import-alias convention (`import x_pb2 as x__pb2`), not the pyi-stub generator's convention (`import x_pb2 as _x_pb2`), so it silently leaves `.pyi` cross-file imports unrewritten; `scripts/fix_pyi_relative_imports.py` covers that gap and runs after protol in `scripts/generate.sh`.
